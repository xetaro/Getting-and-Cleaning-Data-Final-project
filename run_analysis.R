...
x <- file.path("./data" , "UCI HAR Dataset")
...
#### The files that we will use to load data:
...
ActivityTest  <- read.table(file.path(x, "test" , "Y_test.txt" ),header = FALSE)
ActivityTrain <- read.table(file.path(x, "train", "Y_train.txt"),header = FALSE)

#### Read the Subject files

SubjectTrain <- read.table(file.path(x, "train", "subject_train.txt"),header = FALSE)
SubjectTest  <- read.table(file.path(x, "test" , "subject_test.txt"),header = FALSE)

#### Read Fearures files

FeaturesTest  <- read.table(file.path(x, "test" , "X_test.txt" ),header = FALSE)
FeaturesTrain <- read.table(file.path(x, "train", "X_train.txt"),header = FALSE)
...
FeaturesNames <- read.table(file.path(x, "features.txt"),head=FALSE)
names(Features)<- FeaturesNames$V2

  ## Merge columns to get the data frame Data for all data

y <- cbind(Subject, Activity)
...
write.table(Result, file = "tidy2.txt",row.name=FALSE) 

controle_file <- read.table("//.../Coursera/Getting and Cleaning Data/UCI HAR Dataset/tidy2.txt")
dim(controle_file)
[1] 181  68
